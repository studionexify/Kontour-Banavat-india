...
async function pull(orgId) {
  ...
  const rows = await rest(
    ...
    + `&kind=in.(${KINDS.join(',')})${filter}&order=updated_at.asc&limit=${PAGE}`
  );
...
export function syncShop() {
  ...
  syncing = (async () => {
    ...
    try {
      ...
      if (isUnknownKind(e)) {
        unsupported = true;
        lastError = 'The database has not been taught the production kinds yet — run supabase/migrations/0004_shop_kinds.sql.';
      } else {
