    ...
    <div class="panel">
      <div class="searchbar">
        <input class="control" type="search" data-q value="${esc(query)}"
               placeholder="Search code, name, spec" aria-label="Search designs">
      </div>
...
function card(d) {
  const dims = [d.w, d.d, d.h].filter(Boolean);
  return `
    <button class="dcard reveal" data-edit="${esc(d.code)}">
      ...
      <div class="dcard-body">
        ...
        ${dims.length === 3 ? `<div class="dcard-dims">${dims.join(' × ')} mm</div>` : ''}
...
export function openDesignSheet({ code = '', onSaved } = {}) {
  ...
  const draft = {
    ...
    baseRate: existing ? existing.baseRate : 0,
    w: existing ? existing.w : 0,
    d: existing ? existing.d : 0,
    ...
    onMount(root) {
      ...
      function readForm() {
        host.querySelectorAll('[data-k]').forEach((inp) => {
          ...
          draft[k] = ['baseRate', 'w', 'd', 'h'].includes(k) ? Number(inp.value) || 0 : inp.value;
        });
      ...
      function paint() {
        host.innerHTML = `
          <section class="qb-sec">
            ...
            ${field('Base rate', `<input class="control" type="number" min="0" data-k="baseRate" value="${draft.baseRate}">`,
                    'Before any finish adjustment, and before GST.')}

            <h3 class="qb-h">Default size (mm)</h3>
            <div class="qline-dims">
              <label>W <input class="control mini-in" type="number" data-k="w" value="${draft.w || ''}"></label>
              <label>D <input class="control mini-in" type="number" data-k="d" value="${draft.d || ''}"></label>
              <label>H <input class="control mini-in" type="number" data-k="h" value="${draft.h || ''}"></label>
            </div>
            ...
            ${field('Specification',
              `<textarea class="control" data-k="spec" rows="4"
                 placeholder="Construction, material, hardware — prints under the item on a quotation">${esc(draft.spec)}</textarea>`)}
